...
import type { IGetSwapQuote } from '../types';
...
export async function getSwapQuote({ chain, from, to, amount, fromAddress, slippage }: IGetSwapQuote) {
	const tokenFrom = from === ethers.constants.AddressZero ? nativeToken : from;
	const tokenTo = to === ethers.constants.AddressZero ? nativeToken : to;
...
export async function swap({
	...
	const data = await getSwapQuote({ chain, from, to, amount, fromAddress, slippage });
