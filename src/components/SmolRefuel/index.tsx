...
const EmbeddedLogger = {
	...
	logAction: (action: string, data?: any) => {
		...
		console.groupCollapsed(`%c🔧 ACTION [${new Date().toLocaleTimeString()}]: ${action}`, EmbeddedLogger.styles.action);
		if (data) {
...
const SmolRefuel = (): React.ReactElement => {
		...
		return {
			async request({ method, params = [] }: { method: any; params: any }) {
				...
				if (!ALLOWED_SIGNATURE_METHODS.includes(method)) {
					EmbeddedLogger.logError('Security violation', `Method ${method} is not allowed in signature provider`);
	 ...
	 */
	const handleChainSwitch = React.useCallback(
		async (payload: ChainSwitchPayload) => {
	 ...
	 */
	const getTokenDecimals = React.useCallback(
	 ...
	 */
	const validateTokenValue = React.useCallback(
		async ({ tokenAddress, amount }: TokenValidationParams): Promise<ValidationResult> => {
			try {
				...
				if (priceData.fromTokenPrice && priceData.fromTokenPrice > 0) {
					...
					if (valueInUSD > MAX_USD) {
						return {
							safe: false,
							reason: `Transaction value of $${valueInUSD.toFixed(2)} exceeds maximum of $${MAX_USD}`
						};
					}
				}
				return { safe: true };
	 ...
	 */
	const validateTransaction = React.useCallback(
		async (txParams: TransactionParams): Promise<ValidationResult> => {
			try {
	...
	const handleTransaction = React.useCallback(
		async (payload: TransactionRequestPayload) => {
			...
			try {
				...
				if (
					txParams.value &&
					BigInt(txParams.value) > BigInt(0) &&
					txParams.data &&
					txParams.data !== '0x' &&
					txParams.data !== ''
				) {
	...
	React.useEffect(() => {
		...
		const handleMessage = async (event: MessageEvent) => {
			...
			if (process.env.NODE_ENV !== 'development' && event.origin !== 'https://smolrefuel.com') {
