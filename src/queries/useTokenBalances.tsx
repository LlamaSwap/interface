...
async function getTokensBalancesAndPrices(address: string, chainId: any, chainName: string) {
	...
	const tokensToPrice: string[] = balances.balances
		.filter((b) => b.whitelist)
		.map((a) => chainName + ':' + a.address)
		.concat(gasToken);
	const pricePromises: any[] = [];
	for (let i = 0; i < tokensToPrice.length; i += 100) {
		pricePromises.push(
			fetch(`https://coins.llama.fi/prices/current/${tokensToPrice.slice(i, i + 100).join(',')}`).then((r) => r.json())
		);
	}

	const prices = (await Promise.all(pricePromises)).reduce(
		(all, prom) => ({
			...all,
			...prom.coins
		}),
...
const getBalances = async (address, chainId): Promise<Balances> => {
	...
	try {
		...
		const finalBalances = balances.balances
			.concat([
				{
			...
			.reduce((all: Balances, t: any) => {
				...
				all[t.address.toLowerCase()] = {
					...
					balanceUSD:
						price.price !== undefined && t.total_amount != null
							? (price.price * t.total_amount) / 10 ** price.decimals
							: 0
				};
				return all;
			}, {});
